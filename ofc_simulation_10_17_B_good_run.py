...
class FulfillmentCenter:
    def __init__(self, env, storage_type, inbound_delivery_daily, shift_schedule, num_packing_stations, logging=False):
        ...
        for i in range(1, 53):
            self.inbound_delivery_weekly[i] = {
                'tshirt':0,
                'hoodie':0,
                'spants':0,
                'sneaks':0}
    ...
    def get_week_num(self):
        return self.get_datetime(86400*3).isocalendar()[1] - warehouse.week_offset
...
def save_logs(env, warehouse, sim_run_name, save_rate):
    ...
    while(True):
        # save logs to file (replacing previous logs) at specified save rate
...
def picker(env, warehouse, name):
    ...
    unfullfilled orders, if any are available the picker engages in the picking
...
for i in range(1,53):
    if i == 1:  # DECISION: Special schedule for week 1 (Applies to first Wednesday of simulation)
        ...
        # Adding additional amount in first weekly order to replenish inventory after unavoidable initial stockout
        # Extra amounts are shown added to Monday's order quantities
            'Sunday':   {'tshirt':3675,'hoodie':1828,'spants':1852,'sneaks': 918},
...
# use a different stower schedule if only recieving delivery once per week
    ...
    standard_week_schedule = {
        ...
        'Monday': {
            'morning':   {'num_pickers':  7, 'num_stowers':   0, 'num_packers': 2},
            'afternoon': {'num_pickers':  7, 'num_stowers':  30, 'num_packers': 2},
            'evening':   {'num_pickers':  7, 'num_stowers':  30, 'num_packers': 2}},
        'Tuesday': {
            'morning':   {'num_pickers':  5, 'num_stowers':   0, 'num_packers': 2},
    ...
    for i in range(1,53):
        if i == 1: # DECISION: Special schedule for week 1 (Applies to first Wednesday of simulation)
            shift_schedule[i] = {
                ...
                'Monday': {
                    'morning':   {'num_pickers':  7, 'num_stowers':   0, 'num_packers': 2},
                    'afternoon': {'num_pickers':  7, 'num_stowers':  40, 'num_packers': 2},
                    'evening':   {'num_pickers':  7, 'num_stowers':  40, 'num_packers': 2}},
                'Tuesday': {
                    'morning':   {'num_pickers':  5, 'num_stowers':   0, 'num_packers': 2},
                    'afternoon': {'num_pickers':  5, 'num_stowers':   0, 'num_packers': 2},
                    'evening':   {'num_pickers':  5, 'num_stowers':   0, 'num_packers': 2}},
...
# SIMULATION DECISION:
sim_until = 86400*7*10 # 1 day = 86400 seconds
...
env.process(save_logs(env, warehouse, sim_run_name, 86401*3))
#===============================================================================
...
# logging ===============================================================================================
# save logs to csv one last time
if warehouse.logging:
    warehouse.write_sim_logs(env, sim_run_name)

    # calculate total profit and save a log for the run's final results
    total_profit = (
        warehouse.gross_profit -
        (
            warehouse.delivery_expense +
            warehouse.delivery_expense_returns +
            warehouse.labor_expense +
            warehouse.labor_expense_overtime +
            warehouse.inventory_holding_cost +
            warehouse.lost_sales_penalty +
            warehouse.fixed_cost_packing_stations
        )
    )
    with open(f'{folder_path}\\profit_{total_profit}.txt', 'w') as f:
            print(f'total_gross_profit: {warehouse.gross_profit}', file=f)
            print(f'total_delivery_expense: {warehouse.delivery_expense}', file=f)
            print(f'total_delivery_expense_returns: {warehouse.delivery_expense_returns}', file=f)
            print(f'total_labor_expense: {warehouse.labor_expense}', file=f)
            print(f'total_labor_expense_overtime: {warehouse.labor_expense_overtime}', file=f)
            print(f'total_inventory_holding_cost: {warehouse.inventory_holding_cost}', file=f)
            print(f'total_lost_sales_penalty: {warehouse.lost_sales_penalty}', file=f)
            print(f'fixed_cost_packing_stations: {warehouse.fixed_cost_packing_stations}', file=f)
            print(f'fixed_cost_facilities: {warehouse.fixed_cost_facilities}', file=f)
            print(f'===============================================', file=f)
