...
# TODO: Handle inventory cost!!
# TODO: QUESTION: Do we prorate inventory holding cost? (daily, hourly, every second?)
# TODO: QUESTION: How do we pay overtime out? (by the second? half hour hour?)
...
class CustomerOrder:
    def __init__(self, env, warehouse, idx, order_details):
        ...
        """
        # order will be cancelled when age hits order expiration
        self.ORDER_EXPIRATION = 10000000000 # TODO: official value: 72 hours

        # order detail properties
        self.idx = idx
        # TODO: QUESTION: 'OrderId' from csv, do we need it?
...
class FullfillmentCenter:
    def __init__(self, env):
        ...
        self.inventory = RandomInventoryStorage(self.env, self.initial_qtys)

        # DECISION: delivery schedule over 52 weeks, does not change during sim
        self.inbound_shipment_daily = {
            'Monday':   {'tshirt':100,'hoodie':100,'spants':100,'sneaks':100},
            'Tuesday':  {'tshirt':200,'hoodie':200,'spants':200,'sneaks':200},
            'Wednesday':{'tshirt':300,'hoodie':300,'spants':300,'sneaks':300},
            'Thursday': {'tshirt':600,'hoodie':300,'spants':300,'sneaks':200},
            'Friday':   {'tshirt':500,'hoodie':500,'spants':500,'sneaks':500},
            'Saturday': {'tshirt':600,'hoodie':600,'spants':600,'sneaks':600},
            'Sunday':   {'tshirt':700,'hoodie':700,'spants':700,'sneaks':700}}
        # use daily delivery schedule to calculate a weekly schedule
        self.inbound_shipment_weekly = {
            'tshirt':0,
            'hoodie':0,
            'spants':0,
            'sneaks':0}
        for day in self.inbound_shipment_daily:
            self.inbound_shipment_weekly['tshirt'] = (
                self.inbound_shipment_weekly['tshirt'] +
                self.inbound_shipment_daily[day]['tshirt']) 
            self.inbound_shipment_weekly['hoodie'] = (
                self.inbound_shipment_weekly['hoodie'] +
                self.inbound_shipment_daily[day]['hoodie'])
            self.inbound_shipment_weekly['spants'] = (
                self.inbound_shipment_weekly['spants'] +
                self.inbound_shipment_daily[day]['spants'])
            self.inbound_shipment_weekly['sneaks'] = (
                self.inbound_shipment_weekly['sneaks'] +
                self.inbound_shipment_daily[day]['sneaks'])          
        # inbound shipments will land in inbound_parking, intially empty
        self.inbound_parking = InventoryBin(self.env, 1, {
            'tshirt':0,
            'hoodie':0,
            'spants':0,
            'sneaks':0})
        ...
        self.shift_schedule = {
            'Monday':   {'morning': {'num_pickers': 5, 'num_stowers': 2, 'num_packers': 6},'afternoon': {'num_pickers': 2, 'num_stowers': 2, 'num_packers': 6},'evening': {'num_pickers': 2, 'num_stowers': 2, 'num_packers': 6}},
            'Tuesday':  {'morning': {'num_pickers': 5, 'num_stowers': 2, 'num_packers': 6},'afternoon': {'num_pickers': 2, 'num_stowers': 2, 'num_packers': 6},'evening': {'num_pickers': 2, 'num_stowers': 2, 'num_packers': 6}},
            'Wednesday':{'morning': {'num_pickers': 5, 'num_stowers': 2, 'num_packers': 6},'afternoon': {'num_pickers': 2, 'num_stowers': 2, 'num_packers': 6},'evening': {'num_pickers': 2, 'num_stowers': 2, 'num_packers': 6}},
            'Thursday': {'morning': {'num_pickers': 5, 'num_stowers': 2, 'num_packers': 6},'afternoon': {'num_pickers': 2, 'num_stowers': 2, 'num_packers': 6},'evening': {'num_pickers': 2, 'num_stowers': 2, 'num_packers': 6}},
            'Friday':   {'morning': {'num_pickers': 5, 'num_stowers': 2, 'num_packers': 6},'afternoon': {'num_pickers': 2, 'num_stowers': 2, 'num_packers': 6},'evening': {'num_pickers': 2, 'num_stowers': 2, 'num_packers': 6}},
            'Saturday': {'morning': {'num_pickers': 5, 'num_stowers': 2, 'num_packers': 6},'afternoon': {'num_pickers': 2, 'num_stowers': 2, 'num_packers': 6},'evening': {'num_pickers': 2, 'num_stowers': 2, 'num_packers': 6}},
            'Sunday':   {'morning': {'num_pickers': 5, 'num_stowers': 2, 'num_packers': 6},'afternoon': {'num_pickers': 2, 'num_stowers': 2, 'num_packers': 6},'evening': {'num_pickers': 2, 'num_stowers': 2, 'num_packers': 6}}}
        self.shift_length = 100 # TODO: 8 hours (8*60*60) 
        ...
        for i in range(1, self.num_packing_station+1):
            self.packing_stations[i] = PackingStation(self.env, i)
...
# TODO: NEED TO LIMIT PARKING AREA CAPACITY TO 50,000 LBS
def inbound_recieving_dock(env, warehouse, freq):
    ...
    """
    # calulate first inbound recieving time
    wait = 0 # TODO: 9:00am (9*60*60)
    ...
    if freq == 'daily':
        ...
        shipment_schedule = warehouse.inbound_shipment_daily[weekday]
        # set time between deliveries
        wait = 200 # TODO: 24hrs (24*60*60)
    elif freq == 'weekly':
        shipment_schedule = warehouse.inbound_shipment_weekly
        # set time between deliveries
        wait = 700 # TODO: 7days (7*24*60*60)
...
def packer(env, warehouse, assigned_station, name):
    ...
    if len(order_queue.items) > 0:
        ...
        if order.status == CustomerOrderStatus.WAITING_TO_PACK:
            ...
            warehouse.shipped_orders = warehouse.shipped_orders + 1
...
env.process(inbound_recieving_dock(env, warehouse, 'weekly'))
...
print(f'shipped orders: {warehouse.shipped_orders}')
